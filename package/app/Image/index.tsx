...
const SuccessModal: React.FC<{ visible: boolean; message: string; onClose: () => void }> = ({ visible, message, onClose }) => {
  ...
  return visible ? (
    <View className="absolute inset-0 justify-center items-center">
      ...
      <Animated.View
        style={[
          { backgroundColor: Colorizer("#FFFFFF", 1.0), borderRadius: 10, padding: 20, justifyContent: "center", alignItems: "center", width: "80%", shadowColor: Colorizer("#000000", 0.25), shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.25, shadowRadius: 10, elevation: 10 },
          modalStyle
        ]}
      >
        <Ionicons name="checkmark-done-circle" size={50} color={Colorizer("#28a745", 1.0)} /> <Text style={{ fontSize: 24, fontWeight: "bold", marginTop: 10, fontFamily: "Kurale" }}>Success</Text> <Text style={{ textAlign: "center", marginVertical: 10, fontFamily: "Kurale" }}>{message}</Text>
        <TouchableOpacity style={{ backgroundColor: Colorizer("#007BFF", 1.0), paddingVertical: 10, paddingHorizontal: 20, borderRadius: 5, marginTop: 10 }} onPress={onClose}>
...
const ErrorModal: React.FC<{ visible: boolean; message: string; onClose: () => void }> = ({ visible, message, onClose }) => {
  ...
  return visible ? (
    <View className="absolute inset-0 justify-center items-center">
      ...
      >
        <MaterialIcons name="error" size={50} color={Colorizer("#dc3545", 1.0)} /> <Text style={{ fontSize: 24, fontWeight: "bold", marginTop: 10, fontFamily: "Kurale" }}>Error</Text> <Text style={{ textAlign: "center", marginVertical: 10, fontFamily: "Kurale" }}>{message}</Text>
        <TouchableOpacity style={{ backgroundColor: Colorizer("#007BFF", 1.0), paddingVertical: 10, paddingHorizontal: 20, borderRadius: 5, marginTop: 10 }} onPress={onClose}>
          <Text style={{ color: Colorizer("#FFFFFF", 1.0), fontFamily: "Kurale" }}>OK</Text>
        </TouchableOpacity>
      </Animated.View>
...
const DownloadingModal: React.FC<{ visible: boolean; percentage: number; downloadRate: number; eta: number; primaryColor: string }> = ({ visible, percentage, downloadRate, eta, primaryColor }) => {
  ...
  return visible ? (
    <View className="absolute inset-0 justify-center items-center">
      <View style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: Colorizer(primaryColor, 0.3) }} />
      <View style={{ backgroundColor: Colorizer("#FFFFFF", 1.0), borderRadius: 10, padding: 20, justifyContent: "center", alignItems: "center", shadowColor: Colorizer("#000000", 0.25), shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.25, shadowRadius: 10, elevation: 10 }}>
        <ActivityIndicator size="large" color={Colorizer(primaryColor, 1.0)} /> <Text style={{ color: Colorizer(primaryColor, 1.0), fontFamily: "Kurale", fontSize: 18, marginTop: 12 }}>Downloading...</Text>
...
const PreviewImage: React.FC<{ selectedImage: ImageMetadata; screenWidth: number; onViewFullScreen: () => void }> = ({ selectedImage, screenWidth, onViewFullScreen }) => {
  ...
  return (
    <View className="relative">
      ...
      <View style={{ borderTopLeftRadius: 20, borderTopRightRadius: 20, overflow: "hidden", elevation: 4 }}>
        {imageLoading && (
          <View style={{ justifyContent: "center", alignItems: "center", backgroundColor: Colorizer("#0A0A0A", 1.0), height: imageHeight }}>
            <ActivityIndicator size="large" color={Colorizer(selectedImage.primary, 1.0)} /> <Text style={{ marginTop: 10, fontFamily: "Kurale", color: Colorizer(selectedImage.primary, 1.0) }}>Loading HD Image Preview...</Text>
          </View>
        )}
        <Animated.Image
      ...
      <TouchableOpacity onPress={onViewFullScreen} activeOpacity={0.8} style={{ position: "absolute", bottom: 20, right: 20, paddingVertical: 8, paddingHorizontal: 16, borderRadius: 9999, zIndex: 50, backgroundColor: Colorizer(selectedImage.primary, 0.5) }}>
    ...
    <View style={{ flex: 1, backgroundColor: Colorizer("#0A0A0A", 1.0) }}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 20 }}>
        ...
        <View className="p-4 m-3 mt-2.5 border-2 rounded-3xl" style={{ borderColor: Colorizer(selectedImage.primary, 1.0), backgroundColor: Colorizer("#111111", 1.0) }}>
          <Text style={{ fontFamily: "Kurale", color: Colorizer(selectedImage.primary, 1.0), marginBottom: 8, fontSize: 32 }}> {selectedImage.original_file_name.replace(".jpg", "")} </Text>
          {[
            ...
            <View key={index} style={{ flexDirection: "row", alignItems: "center", marginVertical: 4 }}>
              ...
              <Text style={{ marginLeft: 8, fontFamily: "Kurale", color: Colorizer(selectedImage.primary, 1.0) }}>{item.label}:</Text> <Text style={{ marginLeft: 8, fontFamily: "Kurale", color: Colorizer(selectedImage.primary, 1.0) }}>{item.value}</Text>
            </View>
          ))}
          <View className="p-2 m-2 rounded-2xl" style={{ backgroundColor: Colorizer(selectedImage.primary, 0.2) }}>
            <View className="p-2 m-0.5 rounded-t-2xl" style={{ backgroundColor: Colorizer(selectedImage.tertiary, 0.2) }}>
              <Text style={{ marginLeft: 8, fontSize: 20, fontFamily: "Kurale", color: Colorizer(selectedImage.primary, 1.0) }}>Environment:</Text> <Text style={{ marginLeft: 8, fontFamily: "Kurale", color: Colorizer("#FFFFFF", 0.4) }}>{parsedData.environment_prompt}</Text>
            </View>
            <View className="p-2 m-0.5 rounded-b-2xl" style={{ backgroundColor: Colorizer(selectedImage.tertiary, 0.2) }}>
              <Text style={{ marginLeft: 8, marginTop: 8, fontSize: 20, fontFamily: "Kurale", color: Colorizer(selectedImage.primary, 1.0) }}>Moral:</Text> <Text style={{ marginLeft: 8, fontFamily: "Kurale", color: Colorizer("#FFFFFF", 0.4) }}>{parsedData.environment_moral}</Text>
            </View>
          </View>
      ...
      <DownloadingModal visible={isDownloading} percentage={percentage} downloadRate={downloadRate} eta={eta} primaryColor={selectedImage.primary} /> <SuccessModal visible={alertVisible && alertIcon === "checkmark-done-circle"} message={alertMessage} onClose={hideAlert} />
