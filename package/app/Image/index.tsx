  ...
  return visible ? (
    <View className="absolute inset-0 justify-center items-center">
      ...
      <Animated.View style={[{ backgroundColor: HexToRGBA("#FFFFFF", 1.0), borderRadius: 10, padding: 20, justifyContent: "center", alignItems: "center", width: "80%" }, modalStyle]} className="shadow-lg">
        <Ionicons name="checkmark-done-circle" size={50} color={HexToRGBA("#28a745", 1.0)} /> <Text style={{ fontSize: 24, fontWeight: "bold", marginTop: 10 }}>Success</Text>
...
const ErrorModal: React.FC<{ visible: boolean; message: string; onClose: () => void }> = ({ visible, message, onClose }) => {
  ...
  return visible ? (
    <View className="absolute inset-0 justify-center items-center">
      ...
      <Animated.View style={[{ backgroundColor: HexToRGBA("#FFFFFF", 1.0), borderRadius: 10, padding: 20, justifyContent: "center", alignItems: "center", width: "80%" }, modalStyle]} className="shadow-lg">
        <MaterialIcons name="error" size={50} color={HexToRGBA("#dc3545", 1.0)} /> <Text style={{ fontSize: 24, fontWeight: "bold", marginTop: 10 }}>Error</Text>
        <Text style={{ textAlign: "center", marginVertical: 10 }}>{message}</Text>
        <TouchableOpacity style={{ backgroundColor: HexToRGBA("#007BFF", 1.0), paddingVertical: 10, paddingHorizontal: 20, borderRadius: 5, marginTop: 10 }} onPress={onClose} className="mt-2">
          <Text style={{ color: HexToRGBA("#FFFFFF", 1.0) }}>OK</Text>
        </TouchableOpacity>
  ...
  return visible ? (
    <View className="absolute inset-0 justify-center items-center">
      ...
      <View className="bg-white rounded-lg p-5 justify-center items-center shadow-lg">
        <ActivityIndicator size="large" color={HexToRGBA(primaryColor, 1.0)} />
        <Text className="text-lg mt-3" style={{ color: HexToRGBA(primaryColor, 1.0) }}>
          Downloading...
        </Text>
        <Text className="mt-2 text-base" style={{ color: HexToRGBA(primaryColor, 1.0) }}>
        ...
        <View className="flex-row mt-4">
          <View className="flex-1 items-center">
            <Text className="text-sm" style={{ color: HexToRGBA(primaryColor, 1.0) }}>
      ...
      <View className="absolute inset-0 justify-center items-center z-50">
        {!imageLoading && (
          <>
            ...
            <Text className="text-center mb-6 leading-6" style={{ fontFamily: "Kurale", color: HexToRGBA(selectedImage.primary, 1.0) }}>
              picBook™
            </Text>
          </>
        )}
      ...
      <View className="rounded-t-2xl overflow-hidden elevation-4">
        {imageLoading && (
          <View className="justify-center items-center bg-[#0A0A0A]" style={{ height: imageHeight }}>
            <ActivityIndicator size="large" color={HexToRGBA(selectedImage.primary, 1.0)} />
            <Text className="mt-2.5" style={{ fontFamily: "Kurale", color: HexToRGBA(selectedImage.primary, 1.0) }}>
  ...
  return (
    <TouchableOpacity onPress={onDownload} activeOpacity={0.8} className="m-2 rounded-2xl overflow-hidden" style={{ backgroundColor: HexToRGBA(colors.primary, 0.4) }}>
      <Animated.View className="p-3 flex-row items-center justify-center" style={animatedStyle}>
        ...
        <FontAwesome5 name="download" size={15} color={HexToRGBA("#FFFFFF", 1.0)} className="m-2" />
        <Text className="text-base font-bold" style={{ fontFamily: "Kurale", color: HexToRGBA("#FFFFFF", 1.0) }}>
          (Highest Quality)
        </Text>
      </Animated.View>
  ...
  return (
    <View className="flex-1 bg-[#0A0A0A]">
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 20 }}>
        ...
        <View className="p-4 m-3 mt-2.5 border-2 rounded-3xl bg-[#111111]" style={{ borderColor: HexToRGBA(selectedImage.primary, 1.0) }}>
          <Text className="mb-2 text-4xl" style={{ fontFamily: "Kurale", color: HexToRGBA(selectedImage.primary, 1.0) }}>
          ...
          {[
            ...
            <View key={index} className="flex-row items-center my-1">
              ...
              <Text className="ml-2" style={{ fontFamily: "Kurale", color: HexToRGBA(selectedImage.primary, 1.0) }}>
                {item.label}:
              </Text>
              <Text className="ml-2" style={{ fontFamily: "Kurale", color: HexToRGBA(selectedImage.primary, 1.0) }}>
                {item.value}
          ...
          <View className="p-2 m-2 bg-opacity-20 rounded-2xl" style={{ backgroundColor: HexToRGBA(selectedImage.primary, 0.2) }}>
            <Text className="ml-2 text-lg" style={{ fontFamily: "Kurale", color: HexToRGBA(selectedImage.primary, 1.0) }}>
              Environment:
            </Text>
            <Text className="ml-2" style={{ fontFamily: "Kurale", color: HexToRGBA(selectedImage.primary, 1.0) }}>
              {parsedData.environment_prompt}
            </Text>
            <Text className="ml-2 mt-2 text-lg" style={{ fontFamily: "Kurale", color: HexToRGBA(selectedImage.primary, 1.0) }}>
              Moral:
            </Text>
            <Text className="ml-2" style={{ fontFamily: "Kurale", color: HexToRGBA(selectedImage.primary, 1.0) }}>
      ...
      <Modal visible={isFullScreen} transparent={false} onRequestClose={() => setIsFullScreen(false)} presentationStyle="fullScreen" statusBarTranslucent>
        <View className="flex-1 bg-black">
          <TouchableOpacity onPress={() => setIsFullScreen(false)} className="absolute top-14 left-8 z-10">
            <FontAwesome5 name="times" size={50} color={HexToRGBA("#FFFFFF", 1.0)} />
          </TouchableOpacity>
