...
// Define a Category type to map category names to their respective databases
interface Category {
  name: string;
...
const Card: React.FC<CardProps> = memo(({ data }) => {
  ...
  // Reset currentIndex and currentImage when data changes
  useEffect(() => {
    setCurrentIndex(0);
...
const HomePage = (): JSX.Element => {
  ...
  // Removed searchQuery since it's unused
  const [selectedCategory, setSelectedCategory] = useState<string>("Anime"); // Default to "Anime"
  ...
  useEffect(() => {
    ...
    // Function to fetch and process data from the selected category database
    const fetchData = () => {
  ...
  // Since searchQuery is removed, filteredGroups equals groupedData
