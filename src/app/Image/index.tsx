...
const SuccessModal: React.FC<{ visible: boolean; message: string; onClose: () => void }> = ({ visible, message, onClose }) => {
  ...
  return (
    <View className="absolute inset-0 justify-center items-center">
      <Animated.View className="absolute inset-0" style={[{ backgroundColor: Colorizer("#060403", 0.5) }, backdropStyle]} />
      <Animated.View className="w-4/5 rounded-3xl p-5 border-4" style={[{ backgroundColor: Colorizer("#111111", 1.0), borderColor: Colorizer("#25BE8B", 1.0) }, modalStyle]}>
        ...
        style={[
          {
            ...
            borderColor: Colorizer("#C26F2D", 1.0)
          },
          modalStyle
        ]}
      >
        <View className="items-center">
          <MaterialIcons name="error" size={50} color={Colorizer("#C26F2D", 1.0)} />
          <Text className="mt-2.5 text-5xl" style={{ fontFamily: "Lobster_Regular", color: Colorizer("#C26F2D", 1.0) }}>
            Error
          </Text>
          <Text className="my-2.5 text-center text-lg" style={{ fontFamily: "Kurale_Regular", color: Colorizer("#C26F2D", 1.0) }}>
            {message}
          </Text>
          <TouchableOpacity className="mt-2.5 px-5 py-2 rounded-2xl overflow-hidden" style={{ backgroundColor: Colorizer("#C26F2D", 0.4) }} onPress={onClose}>
            <Text className="text-white text-lg" style={{ fontFamily: "Lobster_Regular" }}>
              OK
            </Text>
          </TouchableOpacity>
...
const DownloadingModal: React.FC<{ visible: boolean; percentage: number; downloadRate: number; eta: number; primaryColor: string }> = ({ visible, percentage, downloadRate, eta, primaryColor }) => {
  ...
  return (
    <View className="absolute inset-0 justify-center items-center">
      <View className="absolute inset-0" style={{ backgroundColor: Colorizer("#060403", 0.5) }} />
      <View
        className="w-4/5 rounded-3xl p-5 border-4"
        style={{
          backgroundColor: Colorizer("#111111", 1.0),
  ...
  return (
    <View className="relative">
        ...
        {!imageLoading && (
          <View className="items-center">
            <Animated.View className="justify-center items-center rounded-full">
              <Image style={{ width: 64, height: 64, borderRadius: 32, backgroundColor: Colorizer("#060403", 0.8) }} source={require("@/assets/picWall/picWall.png")} contentFit="contain" />
            </Animated.View>
          </View>
        )}
      </View>
      {imageLoading && (
        <View className="absolute inset-0 z-40 justify-center items-center" style={{ backgroundColor: Colorizer("#060403", 1.0) }}>
      ...
      >
        ...
        <FontAwesome6 name="mobile-button" size={15} color={Colorizer("#F2EFE0", 1.0)} style={{ margin: 4 }} />
        <Text className="text-white text-lg" style={{ fontFamily: "Lobster_Regular" }}>
          (Full-Screen View)
        </Text>
      </TouchableOpacity>
    ...
    <TouchableOpacity onPress={onDownload} activeOpacity={0.8} className="mt-2 rounded-2xl overflow-hidden" style={{ backgroundColor: Colorizer(colors.primary, 0.4) }}>
      <Animated.View className="flex-row items-center justify-center p-3" style={{ transform: [{ scale: scaleValue }] }}>
        <View className="flex-row items-center justify-center">
          ...
          <FontAwesome5 name="download" size={15} color={Colorizer("#F2EFE0", 1.0)} style={{ marginHorizontal: 8 }} />
          <Text className="text-white text-lg" style={{ fontFamily: "Lobster_Regular" }}>
            (Highest Quality)
          </Text>
        </View>
...
const OtherImages: React.FC<OtherImagesProps> = ({ otherImages, setCurrentIndex, primaryColor, tertiaryColor }) => (
  <View className="p-1 my-2 rounded-2xl" style={{ backgroundColor: Colorizer(primaryColor, 0.2) }}>
    <View className="p-1 rounded-2xl" style={{ backgroundColor: Colorizer(tertiaryColor, 0.2) }}>
      <Text className="ml-2 text-xl" style={{ fontFamily: "Lobster_Regular", color: Colorizer("#F2EFE0", 1.0) }}>
        Other Wallpapers:
      </Text>
...
const WallModal: React.FC<WallModalProps> = ({ visible, onComplete, onCancel, wallType, primaryColor }) => {
  ...
  return (
    <View className="absolute inset-0 justify-center items-center">
      <Animated.View className="absolute inset-0" style={[{ backgroundColor: Colorizer("#060403", 0.5) }, backdropStyle]} />
      <Animated.View className="w-4/5 rounded-3xl p-5 border-4" style={[{ backgroundColor: Colorizer("#111111", 1.0), borderColor: Colorizer(primaryColor, 1.0) }, modalStyle]}>
        <View className="items-center">
...
const FullScreenView: React.FC<FullScreenViewProps> = ({ isFullScreen, setIsFullScreen, selectedImage, selectedIndex, data, environment_title }) => {
  ...
  return (
    <Modal visible={isFullScreen} transparent={false} onRequestClose={() => setIsFullScreen(false)} presentationStyle="fullScreen" statusBarTranslucent>
      <View style={{ flex: 1, backgroundColor: Colorizer("#060403", 1.0) }}>
...
const ImagePage = () => {
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: Colorizer("#060403", 1.0) }}>
      ...
      <ScrollView className="flex-1">
        ...
        <View className="p-4 border-2 rounded-3xl" style={{ borderColor: Colorizer(selectedImage.primary, 1.0), backgroundColor: Colorizer("#111111", 1.0) }}>
