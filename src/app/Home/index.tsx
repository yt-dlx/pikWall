        ...
        <TouchableOpacity onPress={() => onImagePress(image.previewLink, index)} className="p-[0.2px] flex-1">
          <View className="relative">
            ...
            <Text
              ...
              style={{ fontFamily: "Kurale_Regular", color: Colorizer("#1A1A1A", 1.0), backgroundColor: Colorizer(image.primary, 1.0) }}
            >
  ...
  return (
    <View className="rounded-b-lg rounded-t-2xl overflow-hidden border" style={{ backgroundColor: Colorizer("#171717", 1.0), borderColor: Colorizer(data.images[currentIndex].primary, 0.4) }}>
        ...
        <TouchableOpacity>
          <View className="relative aspect-[9/16] w-full overflow-hidden">
            ...
            <View className="absolute bottom-0 left-0 right-0 items-center justify-start">
              <Animated.Text
                style={[textStyle, { textAlign: "center", fontFamily: "Kurale_Regular", color: Colorizer("#1A1A1A", 0.8), backgroundColor: Colorizer(data.images[currentIndex].primary, 1.0) }]}
      ...
      <View className="border-t items-center justify-center py-0.5" style={{ backgroundColor: Colorizer(data.images[currentIndex].primary, 1.0) }}>
        <Text style={{ fontFamily: "Dm_Serif_Display_Regular", color: Colorizer("#1A1A1A", 1.0), fontSize: 12, lineHeight: 16 }}>Generated by picWall-AI</Text>
      </View>
    </View>
  );
});
...
function getCategoryIcon(selected: boolean) {
  ...
  const iconColor = selected ? Colorizer("#F5F5F5", 1.0) : Colorizer("#1A1A1A", 1.0);
...
const CategoryButton: React.FC<CategoryButtonExtendedProps> = React.memo(({ category, selected, onPress }) => {
  ...
  return (
    ...
    >
      ...
      <LinearGradient
        colors={selected ? [Colorizer("#5f1314", 1.0), Colorizer("#981e20", 1.0), Colorizer("#BE2528", 1.0)] : [Colorizer("#F5F5F5", 1.0), Colorizer("#D2D2D3", 1.0), Colorizer("#BABABB", 1.0)]}
      ...
      >
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          ...
          <Text style={{ fontFamily: "Caveat_Bold", color: selected ? Colorizer("#F5F5F5", 1.0) : Colorizer("#1A1A1A", 1.0) }} className="ml-1 p-1 text-lg">
            {category}
          </Text>
        </View>
      </LinearGradient>
      <View style={{ alignItems: "center", margin: 2 }}>
        <Animated.View
          style={[{ width: 100, height: 1, backgroundColor: selected ? "#BE2528" : "#F5F5F5", borderRadius: 50 }, useAnimatedStyle(() => ({ transform: [{ translateX: translateX.value }] }))]}
        />
      </View>
    </TouchableOpacity>
  );
        ...
        <View className="pb-1 pt-4 px-2">
          <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "center" }}>
            <Animated.View style={leftIconStyle}>
              <FontAwesome5 name="caret-left" size={24} color="#F5F5F5" />
            </Animated.View>
            <Text style={{ fontFamily: "Lobster_Regular", fontSize: 40, color: "#F5F5F5", textAlign: "center", marginHorizontal: 10 }}>Our Collections</Text>
            <Animated.View style={rightIconStyle}>
              <FontAwesome5 name="caret-right" size={24} color="#F5F5F5" />
            </Animated.View>
          </View>
...
const HomePage = (): JSX.Element => {
  ...
  return (
    <View style={{ backgroundColor: Colorizer("#1A1A1A", 1.0), flex: 1 }} className="relative">
