...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if ("content" in data && "encoding" in data && data.content) {
      // If the response is a file with content and encoding
      fileContentBuffer = Buffer.from(data.content, "base64");
    } else if ("download_url" in data && data.download_url) {
      // If the file is too large, fetch it using the download URL
      ...
      throw new Error(`File content is unavailable for file: ${fileName}`);
    }
